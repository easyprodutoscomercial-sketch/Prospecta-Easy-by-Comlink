'use client';

import { useState, useEffect, useMemo } from 'react';
import Link from 'next/link';

type Association = {
  id: string;
  sigla: string;
  nome_completo: string;
  presidente: string | null;
  telefone: string | null;
  email: string | null;
  website: string | null;
  cidade: string | null;
  estado: string | null;
  endereco: string | null;
  cep: string | null;
  grupo: string | null;
  notas: string | null;
  contacts_count?: number;
  last_interaction_at?: string | null;
};

type StatusFilter = 'ALL' | 'WITH_CONTACTS' | 'NO_CONTACTS' | 'RECENTLY_TALKED' | 'COLD';
type SortMode = 'RECENT' | 'ALPHA' | 'MOST_CONTACTS';

const DAY_MS = 1000 * 60 * 60 * 24;

function daysSince(iso: string | null | undefined): number | null {
  if (!iso) return null;
  const then = new Date(iso).getTime();
  if (Number.isNaN(then)) return null;
  const diff = Date.now() - then;
  return Math.floor(diff / DAY_MS);
}

function formatRelativeDays(iso: string | null | undefined): string {
  if (!iso) return 'nunca contatado';
  const d = daysSince(iso);
  if (d === null) return 'nunca contatado';
  if (d <= 0) return 'hoje';
  if (d === 1) return 'ontem';
  if (d < 30) return `há ${d}d`;
  if (d < 60) return 'há 1 mês';
  if (d < 365) return `há ${Math.floor(d / 30)} meses`;
  const years = Math.floor(d / 365);
  return years === 1 ? 'há 1 ano' : `há ${years} anos`;
}

export default function AssociacoesPage() {
  const [associations, setAssociations] = useState<Association[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [estadoFilter, setEstadoFilter] = useState<string>('ALL');
  const [statusFilter, setStatusFilter] = useState<StatusFilter>('ALL');
  const [sortMode, setSortMode] = useState<SortMode>('RECENT');

  useEffect(() => {
    (async () => {
      try {
        const res = await fetch('/api/associations');
        if (res.ok) {
          const data = await res.json();
          setAssociations(data.associations || []);
        }
      } finally {
        setLoading(false);
      }
    })();
  }, []);

  const estados = useMemo(() => {
    const set = new Set<string>();
    associations.forEach((a) => { if (a.estado) set.add(a.estado); });
    return Array.from(set).sort();
  }, [associations]);

  const headerStats = useMemo(() => {
    const total = associations.length;
    const withContacts = associations.filter((a) => (a.contacts_count || 0) > 0).length;
    const hot = associations.filter((a) => {
      const d = daysSince(a.last_interaction_at);
      return d !== null && d <= 7;
    }).length;
    return { total, withContacts, hot };
  }, [associations]);

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    const list = associations.filter((a) => {
      if (estadoFilter !== 'ALL' && a.estado !== estadoFilter) return false;

      const count = a.contacts_count || 0;
      const d = daysSince(a.last_interaction_at);

      if (statusFilter === 'WITH_CONTACTS' && count === 0) return false;
      if (statusFilter === 'NO_CONTACTS' && count > 0) return false;
      if (statusFilter === 'RECENTLY_TALKED' && (d === null || d > 7)) return false;
      if (statusFilter === 'COLD') {
        if (count === 0) return false;
        if (d === null || d <= 30) return false;
      }

      if (!q) return true;
      return (
        a.sigla.toLowerCase().includes(q) ||
        a.nome_completo.toLowerCase().includes(q) ||
        (a.cidade || '').toLowerCase().includes(q) ||
        (a.presidente || '').toLowerCase().includes(q)
      );
    });

    const sorted = [...list];
    if (sortMode === 'ALPHA') {
      sorted.sort((a, b) => a.sigla.localeCompare(b.sigla, 'pt-BR'));
    } else if (sortMode === 'MOST_CONTACTS') {
      sorted.sort((a, b) => {
        const diff = (b.contacts_count || 0) - (a.contacts_count || 0);
        if (diff !== 0) return diff;
        return a.sigla.localeCompare(b.sigla, 'pt-BR');
      });
    } else {
      sorted.sort((a, b) => {
        const ta = a.last_interaction_at ? new Date(a.last_interaction_at).getTime() : 0;
        const tb = b.last_interaction_at ? new Date(b.last_interaction_at).getTime() : 0;
        if (ta !== tb) return tb - ta;
        return a.sigla.localeCompare(b.sigla, 'pt-BR');
      });
    }
    return sorted;
  }, [associations, search, estadoFilter, statusFilter, sortMode]);

  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white mb-1">Associações</h1>
        <p className="text-sm text-purple-200/60">
          {headerStats.total > 0 ? (
            <>
              <span>{headerStats.total} associações</span>
              <span className="mx-1.5 text-purple-400/40">•</span>
              <span className="text-emerald-300/90">{headerStats.withContacts} com contato</span>
              <span className="mx-1.5 text-purple-400/40">•</span>
              <span className="text-emerald-400">{headerStats.hot} quentes (7d)</span>
            </>
          ) : (
            'Nenhuma associação cadastrada ainda'
          )}
        </p>
      </div>

      {/* Filtros */}
      <div className="mb-6 flex flex-wrap gap-3">
        <input
          type="text"
          placeholder="Buscar por sigla, nome, cidade ou presidente..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="flex-1 min-w-[260px] px-4 py-2.5 bg-[#2a1245] border border-purple-800/30 rounded-lg text-white placeholder-purple-300/40 focus:outline-none focus:border-emerald-500/50 text-sm"
        />
        <select
          value={estadoFilter}
          onChange={(e) => setEstadoFilter(e.target.value)}
          className="px-4 py-2.5 bg-[#2a1245] border border-purple-800/30 rounded-lg text-white focus:outline-none focus:border-emerald-500/50 text-sm"
        >
          <option value="ALL">Todos os estados ({associations.length})</option>
          {estados.map((uf) => {
            const count = associations.filter((a) => a.estado === uf).length;
            return <option key={uf} value={uf}>{uf} ({count})</option>;
          })}
        </select>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as StatusFilter)}
          className="px-4 py-2.5 bg-[#2a1245] border border-purple-800/30 rounded-lg text-white focus:outline-none focus:border-emerald-500/50 text-sm"
        >
          <option value="ALL">Todas</option>
          <option value="WITH_CONTACTS">Com contatos</option>
          <option value="NO_CONTACTS">Sem contatos</option>
          <option value="RECENTLY_TALKED">Falei nos últimos 7 dias</option>
          <option value="COLD">Frias (+30d sem falar)</option>
        </select>
        <select
          value={sortMode}
          onChange={(e) => setSortMode(e.target.value as SortMode)}
          className="px-4 py-2.5 bg-[#2a1245] border border-purple-800/30 rounded-lg text-white focus:outline-none focus:border-emerald-500/50 text-sm"
        >
          <option value="RECENT">Mais recentes primeiro</option>
          <option value="ALPHA">Ordem alfabética</option>
          <option value="MOST_CONTACTS">Mais contatos primeiro</option>
        </select>
      </div>

      {/* Lista */}
      {loading ? (
        <div className="text-center py-16 text-purple-200/50">Carregando...</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 text-purple-200/50">Nenhuma associação encontrada.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((a) => {
            const count = a.contacts_count || 0;
            const hasContacts = count > 0;
            const lastLabel = formatRelativeDays(a.last_interaction_at);
            const neverTalkedButHasContacts = hasContacts && !a.last_interaction_at;

            return (
              <Link
                key={a.id}
                href={`/associacoes/${a.id}`}
                className="bg-[#1e0f35] border border-purple-800/30 rounded-xl p-5 hover:border-emerald-500/40 transition-colors block"
              >
                <div className="flex items-start justify-between mb-3 gap-2">
                  <div className="min-w-0 flex-1">
                    <h3 className="text-base font-bold text-emerald-400 truncate">{a.sigla}</h3>
                    <p className="text-xs text-purple-200/60 line-clamp-2 mt-0.5">{a.nome_completo}</p>
                  </div>
                  <div className="shrink-0 flex flex-col items-end gap-1">
                    {a.estado && (
                      <span className="px-2 py-0.5 text-[10px] font-bold bg-purple-800/40 text-purple-200 rounded">
                        {a.estado}
                      </span>
                    )}
                    {hasContacts ? (
                      <span className="px-2 py-0.5 text-[11px] font-bold bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 rounded">
                        {count} {count === 1 ? 'contato' : 'contatos'}
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 text-[10px] font-semibold bg-purple-800/30 text-purple-300/60 rounded">
                        sem contato
                      </span>
                    )}
                    {hasContacts && (
                      neverTalkedButHasContacts ? (
                        <span className="text-[10px] text-amber-400/90 font-medium">cadastrado, nunca falei</span>
                      ) : (
                        <span className="text-[10px] text-purple-200/60">último: {lastLabel}</span>
                      )
                    )}
                  </div>
                </div>

                <div className="space-y-1.5 text-xs">
                  {a.presidente && (
                    <div className="flex items-start gap-2 text-purple-200/80">
                      <svg className="w-3.5 h-3.5 mt-0.5 shrink-0 text-purple-400/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                      <span className="truncate">{a.presidente}</span>
                    </div>
                  )}
                  {a.cidade && (
                    <div className="flex items-start gap-2 text-purple-200/80">
                      <svg className="w-3.5 h-3.5 mt-0.5 shrink-0 text-purple-400/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                      <span className="truncate">{a.cidade}{a.estado ? ` / ${a.estado}` : ''}</span>
                    </div>
                  )}
                  {a.telefone && (
                    <div className="flex items-start gap-2 text-purple-200/80">
                      <svg className="w-3.5 h-3.5 mt-0.5 shrink-0 text-purple-400/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                      </svg>
                      <a href={`tel:${a.telefone.replace(/\D/g, '')}`} className="truncate hover:text-emerald-400">
                        {a.telefone}
                      </a>
                    </div>
                  )}
                  {a.email && (
                    <div className="flex items-start gap-2 text-purple-200/80">
                      <svg className="w-3.5 h-3.5 mt-0.5 shrink-0 text-purple-400/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      <a href={`mailto:${a.email}`} className="truncate hover:text-emerald-400">{a.email}</a>
                    </div>
                  )}
                  {a.website && (
                    <div className="flex items-start gap-2 text-purple-200/80">
                      <svg className="w-3.5 h-3.5 mt-0.5 shrink-0 text-purple-400/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                      </svg>
                      <a href={a.website} target="_blank" rel="noopener" className="truncate hover:text-emerald-400">
                        {a.website.replace(/^https?:\/\//, '')}
                      </a>
                    </div>
                  )}
                </div>

                {a.grupo && (
                  <div className="mt-3 pt-3 border-t border-purple-800/20">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-purple-400/60">
                      Grupo: {a.grupo}
                    </span>
                  </div>
                )}
              </Link>
            );
          })}
        </div>
      )}
    </div>
  );
}
