...
function EditEventModal({ event, onClose, onSaved }: { event: FairEvent; onClose: () => void; onSaved: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.start_date || !form.end_date) { setError('Preencha nome e datas'); return; }
    if (!form.pipeline_id) { setError('Selecione uma pipeline'); return; }
    setLoading(true);
    setError('');
    try {
      let res: Response;
      if (coverFile) {
        ...
        res = await fetch(`/api/events/${event.id}`, { method: 'PUT', body: fd });
      } else {
          ...
          <div>
            ...
            {coverPreview ? (
              <div className="relative h-36 rounded-lg overflow-hidden border border-purple-700/30">
                ...
                <button type="button" onClick={() => { setCoverFile(null); setCoverPreview(null); }} className="absolute top-2 right-2 p-1 bg-black/50 rounded-full text-white hover:bg-black/70">
