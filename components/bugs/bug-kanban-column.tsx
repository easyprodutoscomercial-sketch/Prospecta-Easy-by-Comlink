...
export function BugKanbanColumn({ id, label, color, bugs }: BugKanbanColumnProps) {
    ...
    >
      ...
      >
        <SortableContext items={bugs.map((b) => b.id)} strategy={verticalListSortingStrategy}>
          {bugs.map((bug) => (
            <BugKanbanCard key={bug.id} bug={bug} />
          ))}
        </SortableContext>
        ...
        {bugs.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-[10px] text-purple-300/30">Nenhum bug</p>
          </div>
        )}
      </div>
    </div>
