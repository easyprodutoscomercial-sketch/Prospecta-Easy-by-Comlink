...
import BugMediaViewer from './bug-media-viewer';

interface BugAttachmentsProps {
  bugId: string;
  attachments: BugAttachment[];
  setAttachments: React.Dispatch<React.SetStateAction<BugAttachment[]>>;
}
...
export default function BugAttachments({ bugId, attachments, setAttachments }: BugAttachmentsProps) {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      const r = await fetch(`/api/bugs/${bugId}/attachments`, { method: 'POST', body: formData });
      if (r.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const r = await fetch(`/api/bugs/${bugId}/attachments/${deleteId}`, { method: 'DELETE' });
      if (r.ok) {
  ...
  return (
    <div>
      ...
      >
        ...
        {uploading ? (
          ...
          <>
            ...
            <p className="text-sm text-neutral-400">Clique ou arraste imagens e videos</p>
            <p className="text-xs text-neutral-600 mt-1">JPG, PNG, GIF, WebP, MP4, WebM (max 25MB)</p>
          </>
        )}
      </div>
            ...
            {images.map((att) => (
              <div key={att.id} className="relative group rounded-lg overflow-hidden bg-[#160b2e] border border-purple-800/20">
                <img
                  ...
                  onClick={() => { setViewerUrl(att.public_url!); setViewerMime(att.mime_type); }}
                />
      ...
      {viewerUrl && (
        <BugMediaViewer url={viewerUrl} mime={viewerMime} onClose={() => setViewerUrl(null)} />
      )}
