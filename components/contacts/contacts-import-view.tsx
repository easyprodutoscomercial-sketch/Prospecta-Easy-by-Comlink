...
const TEMPLATE_ROWS = [
  ...
  },
  {
    ...
    telefone_2: '(21) 97777-2222', contato_2: 'Carlos Souza', cargo_2: 'Compras',
    telefone_3: '(21) 96666-3333', contato_3: 'Fernanda Lima', cargo_3: 'Financeiro',
  },
  {
...
function downloadTemplate(pipelineName?: string) {
  ...
  const sheetName = pipelineName ? pipelineName.slice(0, 31) : 'Contatos';
...
interface ContactsImportViewProps {
  onImportComplete?: () => void;
}

export default function ContactsImportView({ onImportComplete }: ContactsImportViewProps) {
  ...
  const [importPipelineId, setImportPipelineId] = useState<string | null>(selectedPipelineId);

  // Sync when global pipeline changes and user hasn't manually picked yet
  const selectedImportPipeline = pipelines.find((p) => p.id === importPipelineId) || pipelines[0] || null;
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/import', {
        ...
        body: JSON.stringify({ rows, mode, pipeline_id: importPipelineId || selectedPipelineId || undefined }),
      });
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[#1e0f35] border border-purple-800/30 rounded-lg p-5">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-emerald-500/15 border border-emerald-500/20 shrink-0">
            <svg className="w-5 h-5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <div className="flex-1">
            <h2 className="text-sm font-medium text-neutral-100">Pipeline de destino</h2>
            <p className="text-xs text-purple-300/60 mt-0.5">
              Os contatos importados serão adicionados ao pipeline escolhido abaixo.
            </p>
          </div>
          {pipelines.length > 0 ? (
            <select
              value={importPipelineId || ''}
              onChange={(e) => setImportPipelineId(e.target.value || null)}
              className="shrink-0 px-3 py-1.5 text-sm font-medium rounded-lg bg-[#2a1245] text-emerald-400 border border-emerald-500/30 focus:outline-none focus:ring-1 focus:ring-emerald-500/50 cursor-pointer"
            >
              {pipelines.map((p) => (
            ...
            <span className="px-3 py-1.5 text-sm font-medium rounded-lg bg-amber-500/15 text-amber-400 border border-amber-500/30">
      ...
      <div className="bg-[#1e0f35] border border-purple-800/30 rounded-lg p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            onClick={() => downloadTemplate(selectedImportPipeline?.name)}
      ...
      <div className="bg-[#1e0f35] border border-purple-800/30 rounded-lg p-5">
        ...
        <button
          ...
          disabled={!file || loading || !importPipelineId}
