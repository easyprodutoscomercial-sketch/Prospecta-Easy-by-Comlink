...
export default function ContactSidebar({
  ...
  const [editNomeContato, setEditNomeContato] = useState('');
  ...
  const handleAddPhone = async () => {
    ...
    const updated = [...telefones, { phone: newPhone.trim(), nome_contato: newNomeContato.trim(), empresa: newEmpresa.trim(), setor: newSetor.trim(), correto: newCorreto }];
  ...
  const startEditPhone = (idx: number) => {
    const tel = telefones[idx];
    setEditingIdx(idx);
    setEditPhone(tel.phone);
    setEditNomeContato(tel.nome_contato || '');
    setEditEmpresa(tel.empresa || '');
    setEditSetor(tel.setor || '');
  };

  const handleSaveEditPhone = async () => {
    if (editingIdx === null || !onUpdatePhones || !editPhone.trim()) return;
    setSavingPhones(true);
    const updated = telefones.map((t, i) => i === editingIdx ? { ...t, phone: editPhone.trim(), nome_contato: editNomeContato.trim(), empresa: editEmpresa.trim(), setor: editSetor.trim() } : t);
    await onUpdatePhones(updated);
    setEditingIdx(null);
    setSavingPhones(false);
  };
          ...
          <div>
            ...
            {telefones.length > 0 && (
              <div className="space-y-2 mb-2">
                {telefones.map((tel, idx) => (
                  editingIdx === idx ? (
                    <div key={idx} className="space-y-2 p-3 rounded-lg border border-amber-500/30 bg-[#2a1245]/50">
                      <input type="text" placeholder="(00) 00000-0000" value={editPhone} onChange={(e) => setEditPhone(formatPhoneInput(e.target.value))} maxLength={15} className="w-full px-3 py-1.5 text-sm border border-purple-700/30 rounded-lg bg-[#2a1245] text-neutral-200 placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                      <input type="text" placeholder="Nome do contato" value={editNomeContato} onChange={(e) => setEditNomeContato(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-purple-700/30 rounded-lg bg-[#2a1245] text-neutral-200 placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                      <input type="text" placeholder="Empresa" value={editEmpresa} onChange={(e) => setEditEmpresa(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-purple-700/30 rounded-lg bg-[#2a1245] text-neutral-200 placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                      <input type="text" placeholder="Setor / Cargo" value={editSetor} onChange={(e) => setEditSetor(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-purple-700/30 rounded-lg bg-[#2a1245] text-neutral-200 placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                      <div className="flex gap-2">
                        <button onClick={handleSaveEditPhone} disabled={!editPhone.trim() || savingPhones} className="flex-1 px-3 py-1.5 text-xs font-semibold text-white bg-emerald-600 rounded-lg hover:bg-emerald-500 disabled:opacity-40 transition-colors">{savingPhones ? 'Salvando...' : 'Salvar'}</button>
                        <button onClick={cancelEditPhone} className="flex-1 px-3 py-1.5 text-xs font-medium text-neutral-400 border border-purple-700/30 rounded-lg hover:bg-purple-800/20 transition-colors">Cancelar</button>
                      </div>
                    </div>
                  ) : (
                    <div key={idx} className={`p-2 rounded-lg border ${tel.correto ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-red-500/20 bg-red-500/5'}`}>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <button onClick={() => startEditPhone(idx)} className="p-1 text-neutral-500 hover:text-amber-400 transition-colors" title="Editar">
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                          </button>
                          <a href={`https://wa.me/55${extractDigits(tel.phone)}`} target="_blank" rel="noopener noreferrer" className="p-1 text-green-500/60 hover:text-green-400 transition-colors" title="WhatsApp">
                            <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                          </a>
                          <a href={`tel:${extractDigits(tel.phone)}`} className="p-1 text-purple-400/60 hover:text-purple-300 transition-colors" title="Ligar">
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                          </a>
                          <button onClick={() => handleRemovePhone(idx)} disabled={savingPhones} className="p-1 text-neutral-500 hover:text-red-400 transition-colors" title="Remover">
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                          </button>
                        </div>
                      </div>
                      {/* Empresa / Setor info */}
                      {(tel.empresa || tel.setor) && (
                        <div className="mt-1 ml-7 flex items-center gap-2 text-[11px] text-neutral-500">
                          {tel.empresa && <span>{tel.empresa}</span>}
                          {tel.empresa && tel.setor && <span>·</span>}
                          {tel.setor && <span>{tel.setor}</span>}
                        </div>
                      )}
                    </div>
                  )
            ...
            {showAddPhone ? (
              <div className="space-y-2 p-3 rounded-lg border border-purple-700/30 bg-[#2a1245]/50">
                ...
                <input
                  type="text"
                  placeholder="Setor / Cargo (opcional)"
                  value={newSetor}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => { setShowAddPhone(false); setNewPhone(''); setNewNomeContato(''); setNewEmpresa(''); setNewSetor(''); setNewCorreto(true); }}
