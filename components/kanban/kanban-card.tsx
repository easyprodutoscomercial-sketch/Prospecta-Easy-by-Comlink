...
export const KanbanCard = memo(function KanbanCard({ contact, overlay, userMap, currentUserId, onClaimContact, onRequestContact, hasPendingRequest, onJumpForward, onJumpBackward, onScheduleMeeting, hasMeeting, canJumpForward: canFwd, canJumpBackward: canBwd, showScheduleMeeting: showMeeting, lastInteractionAt, bulkMode, bulkSelected, onBulkToggle, pipelineType, attachmentCount, isDimmed, isStuck, compact, onCardClick }: KanbanCardProps) {
  ...
  return (
    <div
      ...
      className={`bg-[#1e0f35] rounded-xl border-l-[3px] border cursor-grab select-none transition-[transform,box-shadow,border-color,background-color,opacity] duration-200 hover:-translate-y-0.5 hover:shadow-lg ${
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-neutral-100 truncate leading-tight">{contact.name}</p>
            ...
            <div className="flex items-center gap-1 ml-auto">
              {(contact.whatsapp || contact.phone) && (
                <a
                  ...
                  className="w-8 h-8 sm:w-6 sm:h-6 flex items-center justify-center rounded-md hover:bg-green-500/15 transition-colors"
                  title="WhatsApp"
                >
              ...
              {contact.email && (
                <a
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="w-8 h-8 sm:w-6 sm:h-6 flex items-center justify-center rounded-md hover:bg-blue-500/15 transition-colors"
                  title="Email"
                >
              ...
              {contact.phone && (
                <a
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="w-8 h-8 sm:w-6 sm:h-6 flex items-center justify-center rounded-md hover:bg-purple-500/15 transition-colors"
                  title="Ligar"
                >
            ...
            <div className="flex items-center justify-between mt-2 pt-2 border-t border-purple-800/15">
              {canJumpBackward && onJumpBackward ? (
                <button
                  ...
                  className="flex items-center gap-1 text-[10px] font-bold text-red-400/60 hover:text-red-400 hover:bg-red-500/10 px-2 py-1 rounded-md transition-colors"
                  title="Voltar etapa"
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {canJumpForward && onJumpForward ? (
                <button
                  ...
                  className="flex items-center gap-1 text-[10px] font-bold text-emerald-400/60 hover:text-emerald-400 hover:bg-emerald-500/10 px-2 py-1 rounded-md transition-colors"
                  title="Avancar etapa"
                >
                  Avancar
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
